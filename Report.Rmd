...
## Market Value and Company Tenure

There is also a slight relationship between the time a CEO has been working for a company and that company's market value, as shown in the graph below:

```{r}
mktval_v_profits <- ggplot(data = CEO_complete, aes(x = CEO_complete$comten, 
                                           y = CEO_complete$mktval)) +
  geom_point(position = position_jitter(width = 0, height = 0.25), alpha=.5) + geom_smooth() +
  xlab("Company Market Value in USD (Millions) on Log Scale") +
  ylab("Company Profits in USD (Millions) on Log Scale") +
  theme(legend.position = c(1,1), legend.justification= c(1,1), legend.background = element_blank(),
...
mktval_v_profits
cor(CEO_complete$comten,CEO_complete$mktval)
```

There is also a slight relationship between the age of a CEO and the company's market value:

```{r}
mktval_v_profits <- ggplot(data = CEO_complete, aes(x = CEO_complete$age, 
                                           y = CEO_complete$mktval)) +
  geom_point(position = position_jitter(width = 0, height = 0.25), alpha=.5) + geom_smooth() +
  xlab("Company Market Value in USD (Millions) on Log Scale") +
  ylab("Company Profits in USD (Millions) on Log Scale") +
...
mktval_v_profits
cor(CEO_complete$age,CEO_complete$mktval)
```
