...
levels(CEO_complete$college) <- c(0,1)
...
## Salary and Company Profitability

There is an undeniably strong relationship between the CEO Salary and Company Profitability, as shown in the plot below:

```{r}
mktval_v_profits <- ggplot(data = CEO_complete, aes(x = log10(CEO_complete$salary), 
                                           y = log10(CEO_complete$profits + 500))) +
  geom_point(position = position_jitter(width = 0, height = 0.25), alpha=.5) + geom_smooth() +
  xlab("CEO Salary in USD (Thousands) on Log Scale") +
  ylab("Company Profits in USD (Millions) on Log Scale") +
  theme(legend.position = c(1,1), legend.justification= c(1,1), legend.background = element_blank(),
        plot.title = element_text(hjust = .5)) +
  labs(title = "CEO Salary vs. Company Profits on Log-Log Scale")
  
mktval_v_profits
cor(log10(CEO_complete$salary),log10(CEO_complete$profits + 500))
```

While it is likely that companies with good profitability would reward their CEO with a high salary, it is unlikely that this would be sole reason. It is possible that there is another variable driving both the salary and the profits, just like market value, which is creating a confounding effect.

## Market Value and CEO Experience

There is a slight relationship between the time a CEO has been working for a company and that company's market value, as shown in the graph below:

```{r}
mktval_v_profits <- ggplot(data = CEO_complete, aes(x = CEO_complete$comten, 
                                           y = log10(CEO_complete$mktval))) +
  geom_point(position = position_jitter(width = 0, height = 0.25), alpha=.5) + geom_smooth() +
  xlab("CEO Total Company Tenure in Years") +
  ylab("Company Market Value in USD (Millions) on Log Scale") +
  theme(legend.position = c(1,1), legend.justification= c(1,1), legend.background = element_blank(),
        plot.title = element_text(hjust = .5)) +
  labs(title = "CEO Total Company Tenure vs. Company Market Value on Log Scale")
  
mktval_v_profits
cor(CEO_complete$comten,log10(CEO_complete$mktval))
```

And there is also a slight relationship between the age of a CEO and the company's market value:

```{r}
mktval_v_profits <- ggplot(data = CEO_complete, aes(x = log10(CEO_complete$age), 
                                           y = log10(CEO_complete$mktval))) +
  geom_point(position = position_jitter(width = 0, height = 0.25), alpha=.5) + geom_smooth() +
  xlab("CEO Age in Years on Log Scale") +
  ylab("Company Market Value in USD (Millions) on Log Scale") +
  theme(legend.position = c(1,1), legend.justification= c(1,1), legend.background = element_blank(),
        plot.title = element_text(hjust = .5)) +
  labs(title = "CEO Age vs. Company Market Value on Log-Log Scale")
  
mktval_v_profits
cor(log10(CEO_complete$age),log10(CEO_complete$mktval))
```

Therefore, given that both the age and the time he has been working for that company are both measures for the CEO's experience, it could be plausible that CEOs who are more experienced in the company's business tend to help drive its market value.
